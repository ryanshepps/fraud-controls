...
private fun Map<String, AttributeValue>.toDecisionAuditRowContract(): DecisionAuditRowContract =
...
private fun Map<String, AttributeValue>.requiredString(name: String): String =
    requireNotNull(this[name]?.s()) { "DynamoDB decision audit row missing string attribute $name" }

private fun Map<String, AttributeValue>.requiredInt(name: String): Int =
    requiredNumber(name).toInt()

private fun Map<String, AttributeValue>.requiredDouble(name: String): Double =
    requiredNumber(name).toDouble()

private fun Map<String, AttributeValue>.requiredNumber(name: String): String =
    requireNotNull(this[name]?.n()) { "DynamoDB decision audit row missing number attribute $name" }

private fun Map<String, AttributeValue>.requiredStringList(name: String): List<String> =
    requireNotNull(this[name]?.l()) { "DynamoDB decision audit row missing list attribute $name" }
        .map { value ->
